// use ClearBitMask with register address and the bits to be masked
#define ClearBitMask(x,y) PCD_WriteReg (x,PCD_ReadReg(x) & ~y)
#define PCD_ClearRegisterBitMask(x,y)  PCD_WriteReg (x,PCD_ReadReg(x) & ~y)

// use SetBitMask with register address and bits to be set
#define SetBitMask(x,y)  PCD_WriteReg (x,PCD_ReadReg(x) | (y))
#define PCD_SetRegisterBitMask(x,y) PCD_WriteReg (x,PCD_ReadReg(x) | (y))
...
void RC522_WriteReg(unsigned char addr, unsigned char data);
void init_pcd (void);
void PCD_Reset (void);
